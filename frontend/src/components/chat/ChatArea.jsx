...
export function ChatArea({ 
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-100/60 dark:bg-[#090d18] relative overflow-hidden transition-colors duration-200">
      ...
      <div className="h-16 px-4 md:px-6 border-b border-slate-200/80 dark:border-white/10 flex items-center justify-between z-10 bg-white/90 dark:bg-[#0d1322]/90 backdrop-blur-md select-none transition-colors duration-200">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {onToggleConversationList && (
            <button
              ...
              className="md:hidden flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-slate-200 text-xs font-semibold transition border border-slate-200/80 dark:border-white/10 shadow-xs"
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-display font-bold text-[15.5px] text-slate-900 dark:text-white tracking-tight">{otherUser?.name}</span>
              {otherUser && (
                <span className="text-[11px] font-semibold px-2 py-0.5 rounded-lg bg-blue-50 dark:bg-blue-600/15 text-blue-700 dark:text-cyan-300 border border-blue-200/80 dark:border-blue-500/30">
                  💼 {otherUser.profession || 'Professional'}
                </span>
              )}
            </div>
            <p className="text-[11.5px] font-medium text-slate-600 dark:text-slate-400 flex items-center gap-1.5">
              {isOtherTyping ? (
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white transition text-xs font-semibold"
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold shadow-md shadow-blue-600/30 transition hover:scale-105 active:scale-95"
            title={activeConversation?.isGroup ? "Join Group HD Video Mesh" : "Start HD Video Call"}
          >
            <Video className="w-4 h-4" />
            <span>{activeConversation?.isGroup ? "Group Video" : "HD Video"}</span>
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3 bg-slate-50/50 dark:bg-[#090d18] transition-colors duration-200">
        ...
        <div className="flex items-center justify-center mb-4">
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-600/10 border border-blue-200 dark:border-blue-500/20 text-[11px] text-slate-600 dark:text-slate-300 shadow-xs">
            <ShieldCheck className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
            <span>HDTalk End-to-End Encrypted Real-Time Chat & HD WebRTC</span>
          </div>
        </div>
      ...
      {/* Input Bar */}
      <div className="p-3 md:p-4 border-t border-slate-200/80 dark:border-white/10 bg-white dark:bg-[#0c1220] relative select-none transition-colors duration-200">
        {/* Attachment Options Menu */}
        {showAttachMenu && (
          <div className="absolute bottom-20 left-4 p-2 bg-white dark:bg-[#141b2e] border border-slate-200 dark:border-white/15 rounded-2xl shadow-xl dark:shadow-2xl z-30 flex flex-col gap-1 w-44 animate-in zoom-in-95">
            <button
        ...
        {showEmojiPicker && (
          <div className="absolute bottom-20 left-12 p-2 bg-white dark:bg-[#141b2e] border border-slate-200 dark:border-white/15 rounded-2xl shadow-xl dark:shadow-2xl z-30 flex flex-wrap gap-2 max-w-xs animate-in zoom-in-95">
        ...
        {showVoiceRecorder ? (
          ...
          <form onSubmit={handleSend} className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2.5 rounded-2xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition"
              title="Attach File"
            >
            ...
            <button
              ...
              className="p-2.5 rounded-2xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-600 dark:text-slate-400 hover:text-amber-500 dark:hover:text-amber-300 transition"
              title="Emojis"
            >
            ...
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-2xl apphitect-input text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none"
            />
            ...
            <button
              ...
              className="p-2.5 rounded-2xl bg-slate-100 dark:bg-white/5 hover:bg-rose-50 dark:hover:bg-rose-500/20 text-slate-600 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 transition"
            ...
            <button
              ...
              className={`p-2.5 rounded-2xl transition duration-150 ${
                text.trim()
                  ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-md shadow-blue-600/30 hover:scale-105 active:scale-95'
